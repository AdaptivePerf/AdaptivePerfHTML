...
def create_html(results_path):
    ...
    script_files = (Path(__file__).resolve().parent / 'static').glob('vis-timeline-graph2d.min*')

    for script_file in script_files:
        shutil.copy(script_file, path)
...
def main():
    if len(sys.argv) != 2 or sys.argv[1] in ['-h', '--help']:
        ...
        print('You can also run adaptiveperf-html with "-h"/"--help" to print this '
